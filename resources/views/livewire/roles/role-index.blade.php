<div>
    ...
    <div>
        ...
        @endsession
        @can('role.create')
        <a href="{{route('roles.create')}}" class="bg-green-500 hover:bg-green-600 text-white text-xs px-3 py-1 rounded mr-2">Create Role</a>
        @endcan
        <div class="overflow-x-auto rounded-lg shadow mt-4">
            <table class="min-w-full divide-y divide-gray-200 text-sm">
                ...
                <tr>
                    ...
                    <th class="px-6 py-3 text-left font-medium text-gray-600">Permissions</th>
                ...
                @foreach($roles as $role)
                    <tr>
                        ...
                        <td class="px-6 py-4 text-center">
                            <a href="{{route('roles.show',[$role->id])}}"  class="bg-gray-500 hover:bg-gray-600 text-white text-xs px-3 py-1 rounded mr-2">Show</a>
                            @can('role.edit')
                            <a href="{{route('roles.edit',[$role->id])}}"  class="bg-blue-500 hover:bg-blue-600 text-white text-xs px-3 py-1 rounded mr-2">Edit</a>
                            @endcan
                            @can('role.delete')
                            <button wire:click="delete({{$role->id}})" wire:confirm="Are you sure to remove this Role?" class="bg-red-500 hover:bg-red-600 text-white text-xs px-3 py-1 rounded">Delete</button>
                            @endcan
                        </td>
                    </tr>
                @endforeach
